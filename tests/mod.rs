...
fn print_exception(env: &mut JNIEnv) -> jni::errors::Result<()> {
...
fn vm_creation_and_object_usage() {
    ...
    // let mut env = vm.attach_current_thread().expect("can't get vm env");
    //
    // User::initNative();
    //
    // assert_eq!(User::nullableString(&mut env, None).expect("can't get nullable string"), None);
    // assert_eq!(User::nullableString(&mut env, Some("hello!".into())).expect("can't get nullable string"), Some("hello!".into()));
    // assert_eq!(User::nullableStringUnchecked(&mut env, None), None);
    // assert_eq!(User::nullableStringUnchecked(&mut env, Some("hello!".into())), Some("hello!".into()));
    //
    // assert_eq!(User::nullableDouble(&mut env, None).expect("can't get nullable double"), 0f64);
    // assert_eq!(User::nullableDouble(&mut env, Some(4.2f64)).expect("can't get nullable double"), 4.2f64);
    // assert_eq!(User::nullableDoubleUnchecked(&mut env, None), 0f64);
    // assert_eq!(User::nullableDoubleUnchecked(&mut env, Some(4.2f64)), 4.2f64);
    //
    // let count = User::getTotalUsersCount(&mut env)
    //     .or_else(|e| {
    //         let _ = print_exception(&mut env);
    //         Err(e)
    //     })
    //     .expect("can't get user count");
    //
    // assert_eq!(count, 0);
    // assert_eq!(User::getTotalUsersCountUnchecked(&mut env), 0);
    //
    // let u = User::new(&mut env, "user".into(), "password".into()).expect("can't create user instance");
    // let u_unchecked = User::newUnchecked(&mut env, "user".into());
    //
    // let count = User::getTotalUsersCount(&mut env)
    //     .or_else(|e| {
    //         let _ = print_exception(&mut env);
    ...
    //     .expect("can't get user count");
    // assert_eq!(count, 2);
    // assert_eq!(User::getTotalUsersCountUnchecked(&mut env), 2);
    //
    // assert_eq!(
    //     u.getPassword(&mut env).expect("can't get user password"),
    //     "password"
    // );
    //
    // assert_eq!(
    //     u.getPasswordUnchecked(&mut env),
    //     "password"
    // );
    //
    // assert_eq!(u_unchecked.toString(&mut env), "User{username='user', password='user_pass'}");
    //
    // assert_eq!(
    //     u.multipleParameters(&mut env, 10, "test".to_string())
    //         .expect("Can't test multipleParameters"),
    //     "test"
    // );
    //
    // assert_eq!(
    //     u.multipleParametersUnchecked(&mut env, 10, "test".to_string()),
    //     "test"
    // );
    //
    ...
    //     User::new(&mut env, login.into(), password.into()).expect("can't create user instance")
    // };
    ...
    // let mut borrow_user = create_user("borrow", "42");
    ...
    // assert_eq!("42__", borrow_user.getPassword(&mut env).expect("unable to get password"));
    // // We actually reinitialize data field here
    // assert_eq!("42__", User::cloneUser(&mut env, &borrow_user).password);
    ...
    // );
    //
    // assert_eq!(borrow_user.toString(&mut env), "User{username='borrow__', password='42__'}");
    // assert_eq!(borrow_user_opt.toString(&mut env), "User{username='borrow_opt____', password='42____'}");
    //
    ...
    //         u.typeOverrideJava(
    //             &mut env, <f64 as JavaValue>::autobox(4.2f64, &mut env)
    //         ).expect("unable to call typeOverrideJava"), &mut env
    //     ), -4.2f64);
    // assert_eq!(
    //     <f64 as JavaValue>::unbox(
